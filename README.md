...
The public repo does not include the private CSV inputs or generated result tables. Protected raw ADNI datasets, derived CSV inputs, raw PLINK files, and raw fMRIPrep derivatives are not distributed here; the helper scripts for regenerating connectivity and GWAS-derived inputs are still included under `scripts/fmri/` and `scripts/gwas/`.
...
├── data/                            # private local inputs (not versioned)
├── requirements.txt
├── pyproject.toml
...
After cloning, place your private CSV inputs into `data/`, then run:
...
That uses the four expected local input files in `data/`, writes phenotype-only candidate edges to `outputs/fmriphenotype/`, and writes the pairwise SNP-connectivity results to `outputs/imggenetics/`.
...
Place these four private inputs in `data/`:
...
`model2_candidate_resilience_edges.csv` is not treated as a public input. It is generated by the phenotype-only step and then reused by `imggenetics`.
...
If you have placed the expected private inputs in `data/`, the defaults are already correct:
...
- `Table 2 Candidate resilience edges.csv`
...
For the standard private local inputs in `data/`:
...
- The public `data/` folder keeps only a schema README; the actual CSV inputs are private and should remain local.
- The public repo does not include generated `outputs/` tables.
