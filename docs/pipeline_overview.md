...
4. **Phenotype-only subtype modeling**
   ...
   Output: `model2_candidate_resilience_edges.csv`, manuscript-ready `Table 2 Candidate resilience edges.csv`, plus related diagnostic tables
...
For the public repo, place these private local inputs in `data/`:
...
`model2_candidate_resilience_edges.csv` is generated by stage 4 and then reused by stage 5. The public repo does not include the private `data/*.csv` inputs or the generated `outputs/` tables. All runnable stages accept explicit input and output paths so the workflow can be reproduced without editing source files.
