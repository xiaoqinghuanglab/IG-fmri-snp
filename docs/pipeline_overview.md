...
`model2_candidate_resilience_edges.csv` is generated by stage 4 and then reused by stage 5. The cleaned repo intentionally stops at the stage-5 full results tables. All runnable stages accept explicit input and output paths so the workflow can be reproduced without editing source files.
